...
export function useIframeSelector(props) {
    ...
    return new Promise((resolve, reject) => {
      try {
        if (!targetIframe || !targetIframe.contentWindow) {
          console.error('[全视界] iframe不可用')
          reject(new Error('iframe不可用'))
          return
        }

        console.log('[全视界] 向iframe发送消息，请求应用选择器')
        ...
        const handleMessage = (event) => {
          if (event.data && 
            ...
            console.log('[全视界] 收到扩展回调:', event.data)
            ...
            if (event.data.success) {
              ...
              console.warn('[全视界] 选择器应用失败:', event.data.error)
        ...
        console.error('[全视界] Chrome扩展调用失败:', error)
        reject(error)
      }
    })
  }
  ...
  const applySelector = async (targetIframe, item) => {
    ...
    console.log('[全视界] 开始应用选择器到指定iframe')
    ...
    if (isElectronEnv) {
      // Electron环境
      try {
        const styleId = `quanshijie-selector-style-${item.id}`
        ...
        console.log('[全视界] Electron环境 - 通过IPC应用选择器')
        const result = await window.electron.executeInIframe(iframeId, code)
        
        if (!result.success) {
          console.warn('[全视界] Electron选择器应用失败:', result.error)
          return false
        }
        
        if (result.result && result.result.hiddenCount !== undefined) {
          console.log('[全视界] Electron选择器应用成功，隐藏了', result.result.hiddenCount, '个元素')
        } else {
          console.log('[全视界] Electron选择器应用成功')
        }
        return true
      } catch (error) {
        console.error('[全视界] 应用选择器失败:', error)
        return false
      }
    } else {
      // 浏览器环境
      try {
        if (!targetIframe.contentDocument) {
          console.warn('[全视界] iframe.contentDocument不可用（跨域iframe）')
        ...
        if (!targetElement) {
          console.warn('[全视界] 未找到选择器对应的元素:', item.targetSelector)
          return
        }
        ...
        console.log('[全视界] 选择器已应用（浏览器环境）')
        return true
      } catch (error) {
        console.warn('[全视界] 无法直接访问iframe（跨域限制）:', error.message)
        return false
      }
    }
  }
  ...
  const applySelectorFullscreen = async () => {
    ...
    console.log('[全视界] 开始应用选择器（Grid模式）')

    const isElectronEnv = window.electron?.isElectron
    console.log('[全视界] 环境检测:', { isElectron: isElectronEnv })
    
    if (isElectronEnv) {
      // Electron环境：通过IPC注入CSS样式到iframe
      try {
        console.log('[全视界] Electron环境 - 应用选择器:', props.item.targetSelector)
        ...
        const code = `
          (function() {
            try {
              ...
              setTimeout(() => {
                console.log('[全视界 iframe] 样式应用后元素状态:', {
              ...
              console.log('[全视界 iframe] 选择器全屏已应用，隐藏了 ' + hiddenCount + ' 个元素');
              return { success: true, hiddenCount: hiddenCount };
            } catch (e) {
              console.error('[全视界 iframe] 错误:', e);
              return { success: false, error: e.message };
        ...
        console.log('[全视界] Electron环境 - 通过IPC应用选择器（Grid模式）')
        ...
        if (!result.success) {
          ...
          console.log('[全视界] Electron选择器应用成功（Grid模式），隐藏了', result.result.hiddenCount, '个元素')
        } else {
          console.log('[全视界] Electron选择器应用成功（Grid模式）')
        }
      } catch (error) {
        console.error('[全视界] 应用选择器失败:', error)
      }
      ...
      console.log('[全视界] 浏览器环境 - 尝试应用选择器')
      console.log('[全视界] iframe引用检查:', {
        hasIframe: !!iframeRef.value,
      ...
      try {
        if (!iframeRef.value) {
          console.warn('[全视界] iframe引用不存在')
          return
        }
        
        if (!iframeRef.value.contentWindow) {
          console.warn('[全视界] iframe.contentWindow不可用')
          return
        }
        ...
        console.log('[全视界] 可以访问iframe内容，开始应用选择器')
        
        const iframeDoc = iframeRef.value.contentDocument
        const targetElement = iframeDoc.querySelector(props.item.targetSelector)
        
        if (!targetElement) {
          console.warn('[全视界] 未找到选择器对应的元素:', props.item.targetSelector)
          return
        }
        
        console.log('[全视界] 直接操作iframe，应用选择器')
        ...
        console.log('[全视界] 已隐藏 ' + hiddenCount + ' 个兄弟元素')
        ...
        console.warn('[全视界] 无法直接访问iframe（跨域限制）:', error.message)
        console.info('[全视界] 提示：如需支持跨域网站，请安装全视界 Chrome扩展')
      }
    }
  }
  ...
  const restoreOriginalStyles = async () => {
    ...
    console.log('[全视界] 开始恢复原始样式（全屏模式）')

    const isElectronEnv = window.electron?.isElectron

    if (isElectronEnv) {
      try {
        console.log('[全视界] 恢复原始样式')
        ...
        const code = `
          (function() {
            try {
              ...
              console.error('[全视界 iframe] 移除样式失败:', e);
              return { success: false, error: e.message };
            }
          })()
        `
        ...
        console.error('[全视界] 恢复原始样式失败:', error)
      }
    } else {
      console.log('[全视界] 浏览器环境 - 尝试恢复原始样式')
      ...
      try {
        if (iframeRef.value && iframeRef.value.contentDocument) {
          ...
          if (style) {
            style.remove()
            console.log('[全视界] 样式已移除')
          }
          
          const hiddenElements = iframeDoc.querySelectorAll('[data-quanshijie-hidden]')
          let restoredCount = 0
          hiddenElements.forEach(el => {
            el.style.display = ''
            el.removeAttribute('data-quanshijie-hidden')
          ...
          console.log('[全视界] 已恢复 ' + restoredCount + ' 个元素')
        } else if (iframeRef.value && iframeRef.value.contentWindow) {
          console.log('[全视界] 跨域iframe，通过扩展恢复样式')
          
          iframeRef.value.contentWindow.postMessage({
            source: 'quanshijie',
            action: 'restoreQuanShiJieStyles'
          }, '*')
        }
      } catch (error) {
        console.warn('[全视界] 恢复样式失败:', error.message)
      }
    }
  }
  ...
  const triggerIframeResize = () => {
    if (iframeRef.value) {
      ...
      console.log('[全视界] 已触发iframe大小微调以刷新布局')
    }
  }
  ...
  const setupIframeLoadListener = () => {
    if (iframeRef.value) {
      iframeRef.value.addEventListener('load', async () => {
        console.log('[全视界] iframe加载完成')
        console.log('[全视界] 当前状态 - 全屏:', props.isFullscreen, '选择器:', props.item.targetSelector)
        
        if (!props.isFullscreen && props.item.targetSelector) {
          console.log('[全视界] Grid模式 + 有选择器，等待1秒后应用选择器')
          setTimeout(async () => {
            ...
            setTimeout(() => {
              ...
              if (onMainIframeReadyCallback) {
                console.log('[全视界] 通知主iframe准备就绪')
                onMainIframeReadyCallback()
              }
            }, 100)
          }, 1000)
        } else {
          console.log('[全视界] 全屏模式或无选择器，显示完整页面')
          // 通知主iframe已准备就绪
          if (onMainIframeReadyCallback) {
            console.log('[全视界] 通知主iframe准备就绪（无选择器）')
            onMainIframeReadyCallback()
          }
        }
      })
  ...
  watch(() => props.isFullscreen, async (newVal, oldVal) => {
    console.log('[全视界 Watch] 全屏状态变化:', newVal)
    console.log('[全视界 Watch] 目标选择器:', props.item.targetSelector)
    console.log('[全视界 Watch] iframe引用:', iframeRef.value)
    console.log('[全视界 Watch] 全屏切换刷新配置:', props.refreshOnFullscreenToggle)
    ...
    if (props.item.targetSelector && iframeRef.value && oldVal !== undefined) {
      if (shouldRefresh) {
        // 刷新模式：重新加载iframe
        console.log('[全视界 Watch] 配置了选择器且允许刷新，刷新iframe以切换显示模式')
        const currentSrc = iframeRef.value.src
        iframeRef.value.src = currentSrc
      } else {
        // 不刷新模式：动态切换选择器应用
        console.log('[全视界 Watch] 配置了选择器但不刷新，动态切换显示模式')
        
        if (newVal) {
          // 进入全屏：恢复被隐藏的元素
          console.log('[全视界 Watch] 进入全屏，恢复原始样式')
          await restoreOriginalStyles()
        } else {
          // 退出全屏：重新应用选择器
          console.log('[全视界 Watch] 退出全屏，重新应用选择器')
          // 等待一小段时间确保DOM稳定
  ...
  onUnmounted(() => {
    console.log('[全视界] WebsiteCard组件卸载')
  })
